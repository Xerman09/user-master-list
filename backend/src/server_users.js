...
const HOST = '0.0.0.0';
const PORT = 3002;
const DIRECTUS = 'http://100.119.3.44:8055/items';
...
function mkStorage(subdir) {
    return multer.diskStorage({
        ...
        filename: (_req, file, cb) => {
            const ts = Date.now();
            const safe = (file.originalname || 'file').replace(/[^a-z0-9.\-_]/gi, '_').toLowerCase();
            cb(null, `${ts}_${safe}`);
        }
    });
}
...
function rewriteLegacyPath(val) {
    ...
    if (BASE_FILE_HOST && smbLike) {
        const cleaned = val.replace(/^file:\/\//i, '').replace(/\\/g, '/').replace(/^\/+/, '');
        return `${BASE_FILE_HOST}/${cleaned}`;
    }
    return null;
}
function bitBufToBool(x) {
    // Directus bit(1)/binary frequently comes as Buffer({data:[0|1]}) or number 0|1 or boolean
    if (x === true || x === false) return !!x;
    if (typeof x === 'number') return x === 1;
    if (typeof x === 'string') return x === '1' || x.toLowerCase() === 'true';
    if (x && typeof x === 'object' && Array.isArray(x.data) && x.data.length) return x.data[0] === 1;
    return false;
}
function transformUser(u) {
    ...
    return {
        ...
        is_deleted: bitBufToBool(u.is_deleted),
        // convenience alias for frontend
        isDeleted: bitBufToBool(u.is_deleted)
    };
}
function transformResp(data) {
    if (Array.isArray(data?.data)) return { ...data, data: data.data.map(transformUser) };
    if (data?.data && typeof data.data === 'object') return { ...data, data: transformUser(data.data) };
    return data;
}

/**
 * Important: on PATCH we *skip* empty-string/null values,
 * so we don't accidentally clear fields when the client doesn't intend to.
 */
...
function sanitizeUserPayload(src, isPatch = false) {
    ...
    for (const [k, v] of Object.entries(src)) {
        if (STRING_FIELDS.has(k)) {
            ...
            out[k] = !!v;
        } else if (k === 'is_deleted') {
            // accept boolean / "true"/"false" / 0/1
            if (isPatch && v === undefined) continue;
            const bool = v === '1' || v === 1 || v === true || v === 'true';
            out[k] = bool ? 1 : 0; // store as 0/1 for bit(1)
        } else {
            if (isPatch && v === undefined) continue;
            out[k] = v;
        }
    }
    // Default on create if not provided
    if (!isPatch && out.is_deleted == null) out.is_deleted = 0;
    return out;
}
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        const token = Buffer.from(JSON.stringify({ userId: user.user_id, ts: Date.now() })).toString('base64');
        ...
        res.status(err.response?.status || 502).json({ error: 'Login failed', detail: err.response?.data || err.message });
    }
});
...
const FIELDS = [
    ...
    'user_sss','user_philhealth','user_tin','user_pagibig',
    'emergency_contact_name','emergency_contact_number',
    'is_deleted' // NEW
].join(',');
...
app.get('/items/user', async (req, res) => {
    const { deleted } = req.query; // '0' | '1' | 'all' | undefined
    try {
        const params = { fields: FIELDS, limit: -1 };

        if (deleted === '0' || deleted === '1') {
            // Prefer server-side filter if Directus supports it
            params['filter[is_deleted][_eq]'] = Number(deleted);
        }

        const { data } = await api.get('/user', { params });
        let payload = transformResp(data);

        // If 'all' do nothing; else if missing server filter, do client filter
        if (deleted === '0' || deleted === '1') {
            payload = {
                ...payload,
                data: (payload.data || []).filter(u => (bitBufToBool(u.is_deleted) ? '1' : '0') === deleted)
            };
        }
        res.json(payload);
    } catch (err) {
        res.status(err.response?.status || 502).json({ error: 'User list failed', detail: err.response?.data || err.message });
    }
});
...
app.get('/items/user/:id', async (req, res) => {
    try {
        ...
        res.status(err.response?.status || 502).json({ error: 'Get user failed', detail: err.response?.data || err.message });
    }
});
...
app.post('/items/user', async (req, res) => {
    try {
        ...
        res.status(err.response?.status || 500).json({ error: 'Create user failed', detail: err.response?.data || err.message });
    }
});

// Update (PATCH with fallback to PUT merge)
app.patch('/items/user/:id', async (req, res) => {
    try {
        ...
        res.status(err.response?.status || 500).json({ error: err.response?.data?.errors?.[0]?.message || 'Failed to update user' });
    }
});
